## Single-flight 核心逻辑拆解

### 使用场景:
业务场景中经常会有缓存的身影，虽然缓存给我们带来了好处，但是缓存带来的问题却不容小觑，常见的有缓存雪崩、缓存穿透、缓存击穿。
今天来说说缓存击穿。发生缓存击穿时，瞬时流量会涌入下游服务或者存储造成极大的冲击甚至打挂。
...
实践的目的是为了理解singleflight的思想，因此在实践过程中简化了代码设计和省去非必要的异常处理，直接用全局变量和全局锁去实现。
```
package main
